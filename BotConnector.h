...
class BotConnector {
public:
  enum RequestType { NONE, SKIP, MOVE, ROTATE, FIRE, SCAN };
  ...
  /* Get the request from the bot. Must be called only once per frame since it
   * establishes a connection each call. */
  Request const &get_request();

  /* Send answer for a simple request. Communicate bot state. */
  void answer_request(unsigned int health, unsigned int numBulletsLeft,
                      unsigned int numKills);

  /* Send answer for a scan request. Communicate also bot state. */
  void answer_scan_request(unsigned int health, unsigned int numBulletsLeft,
                           unsigned int numKills, unsigned int countCircles,
                           unsigned int countBots, unsigned int countBullets);
