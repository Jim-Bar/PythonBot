...
// Initial health, number of bullets left, and number of kills are hard-coded here and in the constructor of BotConnector too.
Bot::Bot(sf::TcpListener& tcpListener, sf::Color color) : m_health(10), m_bulletsLeft(10), m_numKills(0), m_scan(color)
{
  // Connect to the bot.
...
unsigned int
Bot::get_health() const
{
  return m_health;
}

unsigned int
Bot::get_bullets_left() const
{
  return m_bulletsLeft;
}

unsigned int
Bot::get_num_kills() const
{
  return m_numKills;
}
...
sf::Vector2f const&
Bot::get_direction() const
{
  return m_direction;
}

sf::Shape const&
Bot::get_SFML_shape() const
{
  if (is_alive())
    return m_SFMLShape;
  else
...
void
Bot::set_position(float x, float y)
{
...
void
Bot::set_scan(int distance, unsigned int radius)
{
...
Bot::fire_bullet()
{
  ...
  {
    if (m_bulletsLeft == 10)
      m_clockLastBulletLoaded.restart(); // To avoid to shoot 11 times when it was long since the bot fired.
    m_bulletsLeft--;
  }
}
