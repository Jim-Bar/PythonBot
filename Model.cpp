...
Model::Model(unsigned int numBots, unsigned int botPort,
             unsigned int contactPort, unsigned int width, unsigned int height)
  ...
  if (tcpListener.listen(botPort > 0 ? botPort
                                     : (unsigned int)sf::Socket::AnyPort) !=
      sf::Socket::Done)
    std::cerr << "Error : Unable to bind the listener to port " << botPort
              << std::endl;
  ...
  if (contactPort > 0) { // ...if a port for this has been provided.
    ...
    if (connectionStatus == sf::Socket::Done) {
      ...
      char data[6] = {0};
      /* Do not use 'botPort' here as if it is zero (and it should be the case),
      ...
      if (numCharacters > 0) {
        if (contactSocket.send(data, numCharacters) != sf::Socket::Done)
          std::cerr << "Error : Sending bot socket port to the peer failed"
                    << std::endl;
      } else
        std::cerr << "Error : Unable to write the bot socket port to a buffer"
                  << std::endl;
  ...
  sf::Color colors[] = {
      ...
      sf::Color(255, 128, 0), sf::Color(128, 0, 255)}; // (...), Orange, Purple.
  for (unsigned int i(0); i < numBots; i++)
...
unsigned int Model::get_width() const { return m_width; }

unsigned int Model::get_height() const { return m_height; }

std::vector<Object *> const &Model::get_circles() const { return m_circles; }

std::vector<Object *> const &Model::get_alive_bots() const {
  return m_aliveBots;
}

std::vector<Object *> const &Model::get_dead_bots() const { return m_deadBots; }

std::vector<Object *> const &Model::get_scans() const { return m_scans; }

std::vector<Object *> const &Model::get_bullets() const { return m_bullets; }

std::vector<Object *> const &Model::get_bots() const { return m_bots; }

bool Model::collides(Bot const *bot, Bot **botCollision,
  ...
  for (unsigned int i(0); i < m_circles.size(); i++)
    if (bot->distance(m_circles[i]) <=
        2.0f / 3.0f * Bot::botLength + ((Circle *)m_circles[i])->get_radius())
  ...
  *botCollision = 0;
  for (unsigned int i(0); i < m_aliveBots.size(); i++)
    if (bot != m_aliveBots[i] &&
        bot->distance(m_aliveBots[i]) <= 4.0f / 3.0f * Bot::botLength)
...
bool Model::collides(Bullet const *bullet, sf::Vector2f &hitPoint) const {
...
bool Model::collides(Bullet const *bullet, Bot **bot,
                     sf::Vector2f &hitPoint) const {
  // Loop over all the bots.
  for (unsigned int i(0); i < m_aliveBots.size(); i++)
    if (distance(bullet->get_position(), m_aliveBots[i]->get_position()) <=
        bullet->get_radius() + 2.0f / 3.0f * Bot::botLength)
...
void Model::fire_bullet(Bot *bot) {
  bot->fire_bullet();
  m_bullets.push_back(new Bullet(bot));
}

void Model::erase_bullet(unsigned int bulletId) {
  delete m_bullets[bulletId];
  m_bullets.erase(m_bullets.begin() + bulletId);
}

void Model::inflict_damages(Bot *bot) {
  bot->receive_damages();
...
void Model::scan(Bot *bot, bool scanCircles, unsigned int &numCircles,
                 bool scanBots, unsigned int &numBots, bool scanBullets,
  ...
  if (scanBots)
    for (unsigned int i(0); i < m_aliveBots.size(); i++)
      if (bot != m_aliveBots[i] &&
          collides((Bot const *)m_aliveBots[i], &bot->get_scan(), hitPoint))
        numBots++;

  // Scan bullets.
  if (scanBullets)
    for (unsigned int i(0); i < m_bullets.size(); i++)
      if (bot != ((Bullet *)m_bullets[i])->get_bot_origin() &&
          collides(&bot->get_scan(), (Bullet const *)m_bullets[i], hitPoint))
        numBullets++;
...
float Model::distance(sf::Vector2f const &p1, sf::Vector2f const &p2) const {
  return sqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y));
}

void Model::add_bot(sf::TcpListener &tcpListener) {
  sf::Vector2f hitPoint;
  // 'botBuffer' not used, just to be able to call collides().
  Bot *bot(new Bot(tcpListener)), *botBuffer(0);
  do { // Choose a free random position and orientation.
    bot->set_position((float)random_value(m_width),
                      (float)random_value(m_height));
...
bool Model::is_in_world(Bot const *bot, sf::Vector2f &hitPoint) const {
  // FIXME: To improve (think to transform get_vx() in get_vertices()...).
  hitPoint = bot->get_position();
  if (bot->get_v1().x < 0.0f || bot->get_v1().x > m_width ||
      bot->get_v1().y < 0.0f || bot->get_v1().y > m_height)
    return false;
  if (bot->get_v2().x < 0.0f || bot->get_v2().x > m_width ||
      bot->get_v2().y < 0.0f || bot->get_v2().y > m_height)
    return false;
  if (bot->get_v3().x < 0.0f || bot->get_v3().x > m_width ||
      bot->get_v3().y < 0.0f || bot->get_v3().y > m_height)
    return false;

  return true;
}

bool Model::is_in_world(Circle const *circle, sf::Vector2f &hitPoint) const {
...
bool Model::collides(Bot const *bot, Circle const *circle,
                     sf::Vector2f &hitPoint) const {
  if (segment_circle_intersection(bot->get_v1(), bot->get_v2(), circle,
                                  hitPoint))
    return true;
  if (segment_circle_intersection(bot->get_v2(), bot->get_v3(), circle,
                                  hitPoint))
    return true;
  if (segment_circle_intersection(bot->get_v3(), bot->get_v1(), circle,
                                  hitPoint))
    return true;
...
bool Model::collides(Circle const *circle1, Circle const *circle2,
                     sf::Vector2f &hitPoint) const {
  if (distance(circle1->get_position(), circle2->get_position()) <
      circle1->get_radius() + circle2->get_radius()) {
    // Compute hit point position.
    hitPoint = (circle2->get_position() - circle2->get_position()) /
               distance(hitPoint);
...
bool Model::collides(Bot const *bot1, Bot const *bot2,
                     sf::Vector2f &hitPoint) const {
...
bool Model::segment_circle_intersection(sf::Vector2f const &A,
                                        sf::Vector2f const &B,
                                        Circle const *circle,
                                        sf::Vector2f &P) const {
  // http://doswa.com/2009/07/13/circle-segment-intersectioncollision.html
  sf::Vector2f segment(B - A), AToCenter(circle->get_position() - A);
  float segmentLength(distance(segment)),
      projectionLength((AToCenter.x * segment.x + AToCenter.y * segment.y) /
                       segmentLength);
...
bool Model::segments_intersection(sf::Vector2f const &A, sf::Vector2f const &B,
...
unsigned int Model::random_value(unsigned int bound) const {
  unsigned int x(0);
